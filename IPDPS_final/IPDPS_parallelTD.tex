...
guide triangulation (line~\ref{alg:triangulate} in Algorithm~\ref{alg:gavril}), a process of adding edges to make the graph chordal. It is then easy to compute a tree decomposition for the chordal graph using Gavril's construction routine
...
\begin{algorithm}[ht!]
\caption{Gavril's tree decomposition algorithm}
\label{alg:gavril}
\begin{algorithmic}[1]
\Procedure{Gavril}{$G$,$\pi$}\\
$\triangleright$\text{ Graph $G=(V,E)$, $\pi$ a permutation of $V$}
\State Initialize $T = (X, (I,F))$ with $X = I = F = \emptyset$
%\State $H = G$; $n=|V|$
%\For{$i = 1$ to $n$}\Comment{Triangulate $G$}
\State
\textsc H = \textsc{Triangulate}($\pi, G$);\label{alg:triangulate}
%$H$=\textsc{Eliminate}($H$,$\pi_i$,$\{\pi_{i+1}$,{\tiny \ldots},$\pi_n\}$,\texttt{false}); \label{alg:triangulate}
%\EndFor
\State $n = |V|$, $k = 1$, $I = \{1\}$, $X_1 = \{\pi_n\}$
\State $t[\pi_n] =1$ \hfill \text{$\triangleright$ $t$ is an $n$-long array}
\For{$i = n-1$ to $1$} \label{alg:startgavtd} %\Comment{Iterate backwards through $\pi$ to construct $T$}
%BDS Do we need to document the calling syntax of getneighbors?
\State $B_i$=$\text{\textsc{GetNeighbors}}(H,\pi_i,\{\pi_{i+1}$,{\tiny \ldots},$\pi_n\})$; \label{alg:get_nbrs}
\State Find $m = j$ such that $j \leq k$ for all $\pi_k \in B_i$;
\If{$B_i = X_{t[m]}$} \label{gav:line:bag_merge}
\State $X_{t[m]} = X_{t[m]} \cup \{\pi_i\}$;
\State $t[\pi_i] = t[m]$;
\Else
\State $k=k+1$;
\State $I = I \cup \{k\}$; $X_{k} = B_i \cup \{\pi_i\}$; 
\State $F = F \cup \{k, t[m]\}$; \hfill \text{$\triangleright$ update $T$}
\State $t[\pi_i] = k$;
\EndIf
\EndFor
\State \textbf{return}  $T = (X, (I,F))$;\label{alg:endgavtd}
\EndProcedure
\end{algorithmic}
\end{algorithm}
...
uses ParMETIS to partition the nodes into subsets which have known relative positions in the final order, then
...
%%%%%%%%%%%% Algorithm ParMETIS and AMD %%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{algorithm}[ht!]
\caption{Find elimination ordering using ParMETIS (optionally with AMD)}
\label{alg:eo_parmetis}
\begin{algorithmic}[1]
\Procedure{ParFindElimOrder}{G, useAMD}
\text{$\triangleright$ Let $G=(V,E)$, $|V| = n$, and $size/rank$}\\ \text{ be the
 MPI communicator size and task rank}
\State $x = n/size, xm = n \mod size$
\State Let $vtxdist[]$ be an array of $size + 1$, storing 
\State MPI task displacement, with $vtxdist[1] = 0$
\State $vtxdist[i] = vtxdist[i-1] + x \quad \forall \;i \in [2,size + 1]$
\State $V$= $\cup_{i=1}^{size} \alpha_i$; $\alpha_i$ =$\{ v_{ix}, v_{ix + 1},\ldots, v_{x(i+1)-1}\}$
\State \text{$\triangleright$ Local node neighbor info in $adjncy[]$, $xadj[]$}
\State $ \left. \begin{array}{lll}
adjncy & =& \text{concat}(adjncy,N(v_j)) \\
xadj_{v_j} & = & |adjncy|\\
\end{array} \right] \forall \; v_j \in \alpha_i$
\State
\textsc{ParMETIS\_V3\_NodeND}($vtxdist$,$adjncy$,$xadj$)\label{parmetis_serialnodend}
\State Re-ordered vertices of $i^{th}$ MPI task are in
$\alpha^{\prime}_{i}$, 
\State collect vertices into root MPI task.
\State $V^{\prime}$ =  $(\alpha^{\prime}_1, \ldots, \alpha^{\prime}_{size})$ with
\State $\alpha^{\prime}_i$= $(v^{\prime}_{ix}, v^{\prime}_{ix + 1},\ldots , v^{\prime}_{x(i+1) -1})$
\State \text{$\triangleright$ Translate ParMETIS indices back to vertices}
\State $V^{\prime\prime}[v^{\prime}_i] = v_{i} \ where\ i \in [1,n]$; 
\If{!useAMD}
\State \textbf{return} $V^{\prime\prime}$;
\EndIf
\State \text{$\triangleright$ Redistribute $V^{\prime\prime}$ among tasks}
\State $V^{\prime\prime} = \{v^{\prime\prime}_1, v^{\prime\prime}_2, \ldots,
v^{\prime\prime}_n \} = \{\beta_1 \cup \beta_2 \cup \ldots \cup \beta_{size}\}$
\State $\beta_i = \{ v^{\prime\prime}_{ix}, v^{\prime\prime}_{ix +
  1},\ldots, v^{\prime\prime}_{x(i+1) - 1}\}$ \quad $i \in [1,size]$
\State $\pi_i = \textsc{FindElimOrder}(G[\beta_i],AMD)$
\State \textbf{return} $\Pi = (\pi_1, \pi_2, \ldots, \pi_{size})$
\EndProcedure
\end{algorithmic}
\end{algorithm}
%\footnotetext{Concatenate {\it adjncy} vector and $N(v_j)$}
%%%%%% Algorithm ParMETIS and AMD %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


\subsection{A parallel version of Gavril's algorithm}
Once the elimination ordering time has been reduced, Gavril's algorithm (Algorithm~\ref{alg:gavril})
...
algorithm described in Algorithm~\ref{alg:gavril} (for the same elimination ordering). In Algorithm~\ref{alg:par_td}, 
the subroutine \textsc{ReplaceParent} is used to remove redundancy when a parent and child in the unrefined tree have identical bags by essentially removing the child from the tree, making its children now directly inherent from what was their grandparent.
...
When solving problems using dynamic programming and tree decompositions,
the (rooted) decomposition determines a number of data dependencies in
the algorithm. For instance, in Figure~\ref{fig:td_dep} (which shows
a small part of a tree decomposition with children drawn below their
parents) the processing of tree node $B$ cannot finish
until computation at all its children ($D$, $E$ and $F$) has completed.
Therefore we use MADNESS tasks to encapsulate the computation required at each
tree node, and define dependencies using futures.

\begin{figure}[ht!]
\centering
\includegraphics[scale=.30]{figures/td_dep.eps}
\caption{Example of dependencies in a tree}
\label{fig:td_dep}
\end{figure}
...
into the current table. The MADNESS task model enables us to
...
\begin{algorithm}[ht!]
\caption{Update weights at a node $i$ with values
  from child $C$}\label{alg:UpdateTable}
\begin{algorithmic}[1]
\Procedure{UpdateTable}{$i, C$}
\State Let $W_{X_i}[p]$ denote the weight of an independent set $p \subseteq X_i$
\For{all independent sets $s \subseteq X_i$}
\State $W_{X_i}[s]$ = $W_{X_i}[s]$ + $W_{C}[s]$
\EndFor
\EndProcedure
\end{algorithmic}
\end{algorithm}
