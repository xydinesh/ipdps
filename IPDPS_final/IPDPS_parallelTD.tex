...
% CSG
% Maybe put the sextillion quote here?
% CSG  
...
DIMACS (cite?) challenge graph \emph{1dc.512}.  The largest bag in this decomposition would
...
First, the amount of work at each tree node is $O(w^{\log w})$ (cite Wilf).
...
Second, the tree itself can be very unbalanced (see image?) and since we can't process a tree node
...
% CSG 
% would like to insert Gurobi here...
% CSG
...
% Begin Edit Dinesh
% Add information on how we used MPI + pthread for parallel  tree decomposition.
Vertices of the original graph distributed evenly between MPI processes. The vertices assigned to each MPI process further divided based on available pthreads on the system.
% End Edit Dinesh
